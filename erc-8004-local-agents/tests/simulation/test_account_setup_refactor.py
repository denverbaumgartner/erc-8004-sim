...
def test_account_setup_factory_executes_operations(
    ...
    account_setup_info = simulation_env.account_setup_info
    # Verify the factory returned expected metadata
    assert "transaction_hashes" in account_setup_info
    assert "operations_executed" in account_setup_info
    assert (
        account_setup_info["operations_executed"] == 12
    )  # We configured 9 operations (6 original + 3 for agent addresses)

    # Verify all transaction hashes are valid
    for tx_hash in account_setup_info["transaction_hashes"]:
...
def test_eth_transfers_completed(
    ...
    balance_1 = web3.eth.get_balance(accounts[1])
    ...
    balance_2 = web3.eth.get_balance(accounts[2])
...
def test_erc20_transfers_from_deployer(
    ...
    erc20_contract = web3.eth.contract(
        address=contracts["ERC-20"]["address"],
        abi=contracts["ERC-20"]["abi"],
    )
...
def test_operations_executed_in_order(
    ...
    assert len(account_setup_info["transaction_hashes"]) == 12
...
def test_resolve_account_address_patterns(
    ...
    resolved = resolve_account_address("default", accounts, contracts)
    assert resolved == accounts[0]

    # Test account[N] pattern
    resolved = resolve_account_address("account[1]", accounts, contracts)
    assert resolved == accounts[1]

    # Test direct address
    direct_addr = accounts[2]
    resolved = resolve_account_address(direct_addr, accounts, contracts)
    assert resolved == direct_addr

    # Test contract reference
    resolved = resolve_account_address("$contracts.ERC-20.address", accounts, contracts)
    assert resolved == contracts["ERC-20"]["address"]
...
def test_invalid_account_reference(
    ...
    with pytest.raises(ValueError, match="Account index .* out of range"):
        resolve_account_address("account[999]", accounts, contracts)

    # Test invalid contract reference
    with pytest.raises(ValueError, match="Contract reference not found"):
        resolve_account_address("$contracts.NonExistent.address", accounts, contracts)

    # Test invalid format
    with pytest.raises(ValueError, match="Invalid account reference"):
        resolve_account_address("invalid_format", accounts, contracts)
